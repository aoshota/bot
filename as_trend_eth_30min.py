...
def ExtractJustTime_30min(time_stamp):
	flag_just_time_30min = False
	time_stamp_int = time_stamp[-5:-3]
	if (time_stamp_int == "00") or (time_stamp_int == "30"):
		flag_just_time_30min = True
	return flag_just_time_30min
...
# エントリー、決済、損切りの計算
# デモ用 money
def CalcMain(flag_just_time_1min, flag_just_time_30min, data_now,data_sum, data_bb_20, cnt_bb_20, flag_plus, flag_minus, flag_position, money,money_tmp):

	# APIでポジションしているか確認する処理を追加

	kline = [] # デモ用

	if flag_just_time_30min:
		if flag_just_time_1min: # 58~2秒のデータをdata_sumに格納
			...
			if cnt_bb_20 == 20: # BBで使うデータが20個揃ったらBBを計算する
				...
				print(data_bb,end=",")
				with open('30min_eth_BB.csv', mode='a') as f:
...
while True:
	...
	data_sum,data_bb_20,cnt_bb_20,flag_plus,flag_minus,flag_position,money,money_tmp = CalcMain(flag_just_time_1min,flag_just_time_30min,data_now,data_sum,data_bb_20,cnt_bb_20,flag_plus,flag_minus,flag_position,money,money_tmp)
