...
def ExtractJustTime_1hour(time_stamp):
	flag_just_time_1hour = False
	time_stamp_int = time_stamp[-5:-3]
	if time_stamp_int == "00":
		flag_just_time_1hour = True
	return flag_just_time_1hour
...
# エントリー、決済、損切りの計算
# デモ用 money
def CalcMain(flag_just_time_1min,flag_just_time_1hour,data_now,element,money,money_tmp):

	# APIでポジションしているか確認する処理を追加

	kline = [] # デモ用

	if flag_just_time_1hour:
		if flag_just_time_1min: # 58~2秒のデータをdata_sumに格納
			...
			if element['cnt_bb_20'] == 20: # BBで使うデータが20個揃ったらBBを計算する
				...
				with open('1hour_eth_BB_sigma1.csv', mode='a') as f:
				...
				if element['flag_position'] == "BUY" or element['flag_position'] == "SELL": # ポジションが入っている時の処理
					if (int(data_now[1]) <= int(element['data_bb']['mean']) and element['flag_position'] == "BUY" ) or (int(data_now[1]) >= int(element['data_bb']['mean']) and element['flag_position'] == "SELL"):
						...
						if element['flag_position'] == "BUY":
							...
							line_notify("損切り(ロング)1hour:" + str(kline[1]))
							line_notify("\nmoney:" + str(money))
						else:
							money += (money_tmp - kline[0])
							line_notify("損切り(ショート):1hour" + str(kline[0]))
							line_notify("\nmoney:" + str(money))
					...
					if element['flag_plus'] == -1 or element['flag_minus'] == -1:
						# 決済処理
						# デモ用
						while True:
							kline = GetKline()
						...
						if element['flag_position'] == "BUY":
							money += (kline[1] - money_tmp)
							line_notify("決済(ロング)1hour:" + str(kline[1]))
							line_notify("\nmoney:" + str(money))
						else:
							money += (money_tmp - kline[0])
							line_notify("決済(ショート)1hour:" + str(kline[0]))
							line_notify("\nmoney:" + str(money))
						element['flag_position'] = "NO"
						element['flag_plus'] = 0
						element['flag_minus'] = 0
					...
					if element['flag_plus'] == 3 or element['flag_minus'] == 3:
						...
						while True:
							...
							if kline == 0:
								...
								break
						if element['flag_position'] == "BUY":
							line_notify("注文処理(ロング)1hour:" + str(kline[0]))
							line_notify("\nmoney:" + str(money))
						else:
							money_tmp = kline[1]
							line_notify("注文処理(ショート)1hour:" + str(kline[1]))
							line_notify("\nmoney:" + str(money))
						print("注文処理", end=",")
						with open('1hour_eth_BB_sigma1.csv', mode='a') as f:
...
while True:
	...
	flag_just_time_1hour = ExtractJustTime_1hour(data_now[0])
	# デモ用
	# ポジションの有無をAPIで取得するからflaf_positionは消す
	element,money,money_tmp = CalcMain(flag_just_time_1min,flag_just_time_1hour,data_now,element,money,money_tmp)
