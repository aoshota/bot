...
# パラメータ変更箇所
def ExtractJustTime(time_stamp):
...
# エントリー、決済、損切りの計算
# デモ用 money
def CalcMain(flag_just_time, data_now,data_sum, data_bb_20,sma_5 , cnt_bb_20, flag_plus, flag_minus, flag_position, money,money_tmp):
	...
	if flag_just_time: # 58~2秒のデータをdata_sumに格納
		...
		if cnt_bb_20 == 20: # BBで使うデータが20個揃ったらBBを計算する
			...
			sma_5 = CalcBB(data_bb_20[-5:])
			# デモ用
			print("money=" + str(money) + ",close=" + str(data_now[1]),end=",")
			print(data_bb,end=",")
			print("sma_5=" + str(sma_5['mean']),end=",")
			with open('1min_eth_BB_sma5.csv', mode='a') as f:
			...
			if flag_position == "BUY" or flag_position == "SELL": # ポジションが入っている時の処理
				if (int(data_now[1]) <= int(sma_5['mean']) and flag_position == "BUY" ) or (int(data_now[1]) >= int(sma_5['mean']) and flag_position == "SELL"):
					# 決済処理
					# デモ用
					while True:
						kline = GetKline()
	...
	# デモ用　money,money_tmp
	return data_sum,data_bb_20,sma_5,cnt_bb_20,flag_plus,flag_minus,flag_position,money,money_tmp
# ********** function ****************
...
while True:
	...
	data_sum,data_bb_20,sma_5,cnt_bb_20,flag_plus,flag_minus,flag_position,money,money_tmp = CalcMain(flag_just_time,data_now,data_sum,data_bb_20,sma_5,cnt_bb_20,flag_plus,flag_minus,flag_position,money,money_tmp)
